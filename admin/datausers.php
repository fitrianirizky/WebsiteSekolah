...
if ($_POST && isset($_POST['id_users'])) {
    ...
    // Update data user
    $db->update_data_user($id_users, $nama, $username, $password, $role);
...
tr td a[href*="edit_jurusan"],
tr td a[href*="hapus_jurusan"] {
          ...
          <div class="container-fluid">
            ...
            <div class="row">
              ...
              <div class="col-md-12">
                            ...
                            <td>
                                <!-- Tombol Edit (trigger modal) -->
                                    <button class="btn btn-warning btn-sm mb-2" 
                                            ...
                                            data-bs-target="#modalEdit<?= $x['id_users']; ?>">
                                      Edit
                                    </button>

                                    <!-- Modal Edit -->
                                    <div class="modal fade" id="modalEdit<?= $x['id_users']; ?>" 
                                        data-bs-backdrop="static" 
                                        data-bs-keyboard="false" 
                                        tabindex="-1" 
                                        aria-labelledby="labelEdit<?= $x['id_users']; ?>" 
                                        aria-hidden="true">
                                      <div class="modal-dialog modal-lg">
                                        <div class="modal-content">
                                          <form action="datausers.php" method="POST">
                                            <div class="modal-header bg-warning">
                                              <h5 class="modal-title" id="labelEdit<?= $x['id_users']; ?>">Edit Data User</h5>
                                              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body">
                                              <input type="hidden" name="id_users" value="<?= $x['id_users']; ?>">
                                              <div class="mb-3">
                                                <label for="nama<?= $x['id_users']; ?>" class="form-label">Nama Lengkap</label>
                                                <input type="text" class="form-control" id="nama<?= $x['id_users']; ?>" name="nama" value="<?= htmlspecialchars($x['nama']); ?>" required>
                                              </div>
                                              <div class="mb-3">
                                                <label class="form-label">Username</label>
                                                <input type="text" class="form-control" name="username" value="<?= htmlspecialchars($x['username']); ?>" required>
                                              </div>
                                              <div class="mb-3">
                                                <label class="form-label">Password</label>
                                                <input type="password" class="form-control" name="password" value="<?= htmlspecialchars($x['password']); ?>">
                                              </div>
                                              <div class="mb-3">
                                ...
                                <!-- Tombol Hapus (trigger modal) -->
                                    <button class="btn btn-danger btn-sm mb-2" 
                                            data-bs-toggle="modal" 
                                            data-bs-target="#modalHapus<?= $x['id_users']; ?>">
                                      Hapus
                                    </button>
                                    
                                    <!-- Modal Konfirmasi Hapus -->
                                    <div class="modal fade" id="modalHapus<?= $x['id_users']; ?>" 
                                        data-bs-backdrop="static" 
                                        data-bs-keyboard="false" 
                                        tabindex="-1" 
                                        aria-labelledby="labelHapus<?= $x['id_users']; ?>" 
                                        aria-hidden="true">
                                      <div class="modal-dialog">
                                        <div class="modal-content">
                                          <div class="modal-header bg-danger text-white">
                                            <h5 class="modal-title" id="labelHapus<?= $x['id_users']; ?>">Konfirmasi Hapus</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                          </div>
                                          <div class="modal-body">
                                          ...
                                          <div class="modal-footer">
                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                                            <a href="hapus_user.php?id_users=<?= $x['id_users']; ?>" class="btn btn-danger">Hapus</a>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                ...
                <!-- /.card -->
                <a href="tambahusers.php?">Tambah Data</a> </div>
              </div>
              <!-- /.col -->
            </div>
            <!--end::Row-->
